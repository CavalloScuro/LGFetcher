...
async function startup({ id, version, rootURI }, reason) {
  ...
  try {
    Zotero.PreferencePanes.register({
      ...
      scripts: [rootURI + "preferences-ui.js"],
      label: "LGFetcher",
  ...
  // Seed defaults after all UI registration so preference initialization
  // can never prevent the Settings pane or menu commands from loading.
  try {
    const defaults = {
      "extensions.lgfetcher.mirrors": "https://libgen.li\nhttps://libgen.la\nhttps://libgen.gl\nhttps://libgen.bz\nhttps://libgen.vg\nhttps://libgen.is",
      "extensions.lgfetcher.formats.pdf": true,
      "extensions.lgfetcher.formats.epub": true,
      "extensions.lgfetcher.formats.djvu": true,
      "extensions.lgfetcher.formats.mobi": true
    };
    for (const prefName in defaults) {
      if (Zotero.Prefs.get(prefName, true) === undefined) {
        Zotero.Prefs.set(prefName, defaults[prefName], true);
      }
    }
  } catch (e) {
    Zotero.logError(e);
